...
type RestOf<T extends IOffer | NewOffer> = Omit<T, 'fromDate' | 'toDate'> & {
...
export class OfferService {
  ...
  protected convertDateFromClient<T extends IOffer | NewOffer | PartialUpdateOffer>(offer: T): RestOf<T> {
    return {
      ...offer,
      fromDate: offer.fromDate?.toJSON() ?? null,
      toDate: offer.toDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOffer: RestOffer): IOffer {
    return {
      ...restOffer,
      fromDate: restOffer.fromDate ? dayjs(restOffer.fromDate) : undefined,
      toDate: restOffer.toDate ? dayjs(restOffer.toDate) : undefined,
    };
  }
